...
**`list` response columns:** `NAME`, `SOURCE_TYPE`, `SOURCE_URI`, `SOURCE_DATASET_NAME`, `INGESTION_DATABASE`, `INGESTION_STATUS`, `CURRENT_PHASE`, `LAST_ERROR_MESSAGE`, `NEXT_INGESTION`, `ENABLED`, `SCHEDULE`, `CREATED`, `LAST_RUN`, `INGESTION_TARGET_TABLE`.

- `CURRENT_PHASE`: While ingestion is running, shows the current execution phase (e.g., "3/6 Retrieving"). Null when not running.
- `LAST_ERROR_MESSAGE`: When the task has failed, contains a description of the failure including the phase where it occurred (e.g., "Failed in phase 2/6 Retrieving: Data retrieval took too long and was stopped."). Null for non-failed tasks.

**`runs` response columns:** `NAME`, `START_TIME`, `END_TIME`, `STATUS`, `DESCRIPTION`, `ERROR_CODE`, `ERROR_MESSAGE`, `DURATION`, `TRIGGER_TYPE`, `ROWS_LOADED`.

- `DESCRIPTION`: Execution phase at the time of the status update (e.g., "3/6 Retrieving"). Null for completed runs.
- `ERROR_CODE`: Machine-readable error code for failed runs (e.g., `TIMEOUT_ERROR`, `STAGING_ERROR`). Null for successful runs.
- `ERROR_MESSAGE`: Human-readable failure description including the phase where the error occurred. Null for successful runs.
...
|--------|---------|------------|
| **IN PROGRESS** | Ingestion is actively running. Check `CURRENT_PHASE` for progress (e.g., "3/6 Retrieving"). | Enabled |
| **SUCCESS** | Last ingestion completed successfully | Enabled |
| **RETRYING** | Ingestion failed but will automatically retry after a delay. Check `LAST_ERROR_MESSAGE` for the failure reason. | Enabled |
| **FAILED** | Ingestion failed and all retry attempts are exhausted; task auto-disabled. Check `LAST_ERROR_MESSAGE` for the failure reason. | Disabled |
...
- **State transitions:** Create (with schedule) → IN PROGRESS → SUCCESS or RETRYING → FAILED. Suspend → INACTIVE. Resume from FAILED or RETRYING → resets to IN PROGRESS (new ingestion starts).
...
- `status: 'failed'` — discovery failed, check `error` field
...
4. If the user wants monitoring: poll `GDC_TASK_INFO('runs', ...)` periodically and report when status changes from IN PROGRESS to SUCCESS, RETRYING, or FAILED
...
|--------|-----------|-------------------|
| Status = RETRYING | `GDC_TASK_INFO('list')` | Task failed but will automatically retry |
| Status = FAILED | `GDC_TASK_INFO('list')` | Task failed after all retries, auto-disabled |
| LAST_ERROR_MESSAGE | `GDC_TASK_INFO('list')` | Description of the failure, including the phase where it occurred |
| ERROR_CODE + ERROR_MESSAGE on failed run | `GDC_TASK_INFO('runs')` | Machine-readable error code and human-readable failure description |
...
1. **Get the task status and error message:** `GDC_TASK_INFO('list')` — check `INGESTION_STATUS` and `LAST_ERROR_MESSAGE`
2. **Get run history:** `GDC_TASK_INFO('runs', ...)` — check `ERROR_CODE`, `ERROR_MESSAGE`, `ROWS_LOADED`, and the pattern of successes/failures
